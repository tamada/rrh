...
func TestUpdateTrueFalseValue(t *testing.T) {
	var testdata = []struct {
		...
		{lib.RrhAutoDeleteGroup, "True", false, "true"},
		{lib.RrhAutoDeleteGroup, "FALSE", false, "false"},
		{lib.RrhAutoDeleteGroup, "FALSE", false, "false"},
		{lib.RrhAutoDeleteGroup, "YES", true, ""},
		{lib.RrhAutoCreateGroup, "FALSE", false, "false"},
		{lib.RrhAutoCreateGroup, "YES", true, ""},
		{lib.RrhSortOnUpdating, "FALSE", false, "false"},
		{lib.RrhSortOnUpdating, "YES", true, ""},
	}
...
func TestUpdateOnError(t *testing.T) {
	...
	for _, data := range testdata {
		var dbfile = lib.Rollback("../testdata/test_db.json", "../testdata/config.json", func(config *lib.Config, oldDB *lib.Database) {
			if err := config.Update(lib.RrhOnError, data.key); (err == nil) != data.success {
				t.Errorf("%s: set to \"%s\", success: %v", lib.RrhOnError, data.key, data.success)
			}
		})
		defer os.Remove(dbfile)
	}
...
func TestUpdateValue(t *testing.T) {
	var testdata = []struct {
		...
		{lib.RrhConfigPath, "hogehoge", true, ""},
		{lib.RrhHome, "hoge1", false, "hoge1"},
		{lib.RrhDatabasePath, "hoge2", false, "hoge2"},
		{lib.RrhDefaultGroupName, "hoge3", false, "hoge3"},
		{lib.RrhTimeFormat, "not-relative-string", false, "not-relative-string"},
		{"unknown", "hoge4", true, ""},
	}
	for _, td := range testdata {
		var dbfile = lib.Rollback("../testdata/test_db.json", "../testdata/config.json", func(unusedConfig *lib.Config, oldDB *lib.Database) {
